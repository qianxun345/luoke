...
### ✨ 启动流程与热键
1. **屏幕适配**：
   - 方法一：游戏界面分辨率调整到`1600*900`，打开电脑设置，选择“系统” -> “屏幕”，将屏幕缩放改为125%，显示器分辨率为`2560*1440`。
   - 方法二：将屏幕缩放改为125%，自行截取image1.png和image2.png，下载并安装上述ahk，注意image1.png的左上侧与您传送点位置应保持110px个像素，具体查看像素可用微信截屏快捷键`Alt + A`查看。
2. **备战前置**：传送到学院驻地的魔力之源。2号位放置好灵巧的宠物或者迪莫，其他号位放置奇丽花。需要保证人物没有站在`image1.png`中的区域，否则会识别不到。如果地图中没有显示学院驻地那片地方，需要你手动拖动到学院驻地附近，过程中请一直保持地图最大的状态！
3. **启动脚本进程**：在本目录下，**以管理员身份运行** `luoke_pgup.ahk`（或纯净版 `.exe`）。运行成功后系统右下角将出现绿色的特定图标（通常带有一个白色的 `H`）。
4. **开刷 / 停止操作**：
